...
int main(int argc, char** argv) {
    ...
    cv::Mat image = cv::imread("ciri.jpg", cv::IMREAD_GRAYSCALE);
    cv::Mat image_flipped;
    //cv::flip(image, image_flipped, 1);

        // Rotate the image

    cv::rotate(image, image_flipped, cv::ROTATE_180);  // 180 degree rotation
    ...
    double max_dist_sift = 0; double min_dist_sift = 100;
    for( int i = 0; i < descriptors_sift.rows; i++ ) {
        double dist = matches_sift[i].distance;
        if( dist < min_dist_sift ) min_dist_sift = dist;
        if( dist > max_dist_sift ) max_dist_sift = dist;
    }
    ...
    double max_dist_sgloh2 = 0; double min_dist_sgloh2 = 100;
    for( int i = 0; i < descriptors_sgloh2.rows; i++ ) {
        double dist = matches_sgloh2[i].distance;
        if( dist < min_dist_sgloh2 ) min_dist_sgloh2 = dist;
        if( dist > max_dist_sgloh2 ) max_dist_sgloh2 = dist;
    }
